...
def setup_simulation(model, temperature, frictionCoeff=1/unit.picosecond, stepSize=1*unit.femtosecond, minimizeEnergy=False):
    ...
    for force in model.system.getForces():
        if isinstance(force, (openmm.MonteCarloBarostat, openmm.MonteCarloAnisotropicBarostat, openmm.MonteCarloFlexibleBarostat)):
...
class WaterModel():
    def __init__(self, positions, box, water_type='tip4pew', nonbondedCutoff=1, barostat=None, external_field=None):
        if water_type not in ['tip3p', 'tip4pew', 'tip5p', 'spce']:
            print(f'+++ WARNING: Unknown water_type `{water_type}` +++', file=stderr) #might still work
    ...
    def set_barostat(self, barostat, pressure=1*unit.bar, temperature=300*unit.kelvin):
        ...
        for i in reversed(range(self.system.getNumForces())):
            if isinstance(self.system.getForce(i), (openmm.MonteCarloBarostat, openmm.MonteCarloAnisotropicBarostat, openmm.MonteCarloFlexibleBarostat)):
        ...
        if barostat is None:
            ...
            self.system.addForce(openmm.MonteCarloAnisotropicBarostat(3*[pressure], temperature))
        elif barostat == 'tri':
    ...
    def set_external_field(self, external_field):
        ...
        if external_field is None:
            ...
            nonbonded = [f for f in self.system.getForces() if isinstance(f, openmm.NonbondedForce)][0]
