...
def plot_results(
    ...
    if ESSs is not None:
        ...
        print(f"average ESS: {ESSs.mean() / num_samples:.2%} +/- {ESSs.std() / num_samples:.2%}")

    xlim = [0, len(deltaFs)]
    n = n_sigmas  # how many sigmas for errorbar

    plt.plot(deltaFs, ".", c="blue", label="LFEP")
    x = 2 * [deltaFs.mean()]
    plt.fill_between(
        xlim, x - n * deltaFs.std(), x + n * deltaFs.std(), color="blue", alpha=0.3
    )
    ...
    if reference_deltaF is not None:
        ...
        print(f"DeltaF_ref  = {reference_deltaF/sc:.6f} +/- {n*reference_deltaF_std/sc:.6f}")
    plt.xlim(xlim)
    plt.legend()
    plt.show(block=stop_for_plots)
...
np.savez(filename, deltaFs=deltaFs, std_deltaFs=std_deltaFs, ESSs=ESSs, num_samples=num_samples)
